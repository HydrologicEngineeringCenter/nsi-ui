...
export default (opts) => {
  ...
  return {
    ...
    [doInitialize]: () => ({ dispatch, store }) => {
      ...
      const dataSelector = `selectDrawSaveCache${config.type}`;
      const data = store[dataSelector]();

      const source = new VectorSource();
      if(data && data.features && data.features.length){
        const features = geoJSON.readFeaturesFromObject(data, {
      ...
      source.on('change', debounce(store.doDrawSave, 300));
